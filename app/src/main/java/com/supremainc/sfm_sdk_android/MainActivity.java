...
public class MainActivity extends AppCompatActivity {
    ...
    private final int ConnectionType = UART;
    /*
        ...
        public void onReceive(Context context, Intent intent) {
            switch (intent.getAction()) {
                case UsbService.ACTION_USB_PERMISSION_GRANTED: // USB PERMISSION GRANTED
                    ...
                    Log.d("Usb Permission", "USB Permission Granted");
                ...
                case UsbService.ACTION_USB_PERMISSION_NOT_GRANTED: // USB PERMISSION NOT GRANTED
                    ...
                    Log.d("Usb not granted", "USB Permission Not Granted");

                    break;
                case UsbService.ACTION_NO_USB: // NO USB CONNECTED
                    Toast.makeText(context, "No USB connected", Toast.LENGTH_SHORT).show();
                    Log.d("Usb connected", "No USB Connected");
                    Toast.makeText(context, "No USB Connected", Toast.LENGTH_SHORT).show();

                    break;
                case UsbService.ACTION_USB_DISCONNECTED: // USB DISCONNECTED
                    Toast.makeText(context, "USB disconnected", Toast.LENGTH_SHORT).show();
                    Log.d("Usb disconnect", "USB Disconnected");

                    break;
                case UsbService.ACTION_USB_NOT_SUPPORTED: // USB NOT SUPPORTED
                    Toast.makeText(context, "USB device not supported", Toast.LENGTH_SHORT).show();
                    Log.d("Usb unsuported", "USB device not supported");
...
//        Log.d("UF_CommandSendData", String.format("%d %d %d", param[0], size[0], flag[0]));
    ...
    private void Test_Calibrate_and_Reset() {
        final String TAG = "TEST_CALIBRATE";
...
////        Log.d("UF_Command", ret.toString());
//
//                            UF_RET_CODE ret = sdk.UF_CommandEx((byte) 0x11, param, size, flag, msgCallback);
//                            Log.d("UF_CommandEx", ret.toString());
//                            Log.d("UF_CommandEx", String.format("%X %X %X", param[0], size[0], flag[0]));
        try{
            // UF_SendNetworkPacket
            //ret = sdk.UF_SendNetworkPacket((byte) 0x11, (short) 1, 0, param, (byte) 0, 1000);
            //Log.d("UF_SendNetworkPacket", ret.toString());
            ...
            //ret = sdk.UF_Identify(userID, subID);
            byte[] templategabung;
            byte[] templategabung2;
            ret = sdk.UF_ScanTemplate(templateData, templateSize, imageQuality);
            templategabung = templateData;
            ret = sdk.UF_ScanTemplate(templateData, templateSize, imageQuality);
            templategabung2 = templateData;

            byte[] c = new byte[templategabung.length + templategabung2.length];
            System.arraycopy(templategabung, 0, c, 0, templategabung.length);
            System.arraycopy(templategabung2, 0, c, templategabung.length, templategabung2.length);
            //ret = sdk.UF_VerifyHostTemplate(1, 384, templateData);
            if (ret == UF_RET_CODE.UF_RET_SUCCESS) {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        String str = "Verify Success\n";
                        display.append(str);
                        //str = "quality : " + imageQuality[0] + "\n";
                        //display.append(str);
                        str = "Template " + Arrays.toString(c) + "\n";
                        display.append(str);
                        //str = "Template Size " + templateSize[0] + "\n";
                        str = "User ID " + userID[0] + "\n";
                        display.append(str);
                    }
                });
            ...
            Log.d(TAG, "Test_WSQ: template :  " + ret);
        }catch (Exception err){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        UsbManager m = (UsbManager) this.getSystemService(Context.USB_SERVICE);
        HashMap<String, UsbDevice> usbDevices = m.getDeviceList();

        // Method to log all connected USB devices
        if (usbDevices.isEmpty()) {
            Log.d("Connected USB", "No USB devices connected.");
        } else {
            Log.d("Connected USB", "Connected USB devices:");
            for (UsbDevice usb : usbDevices.values()) {
                Log.d("Connected USB", "Device Name: " + usb.getDeviceName() +
                                            "\n Vendor " + usb.getVendorId() +
                                            "\n Product " + usb.getProductId());
                // Add more details as needed, e.g., manufacturer, product ID, etc.
            }
        }
        // for USB connection
        if(this.ConnectionType == USB) {
            ...
            // Initialize system parameter (this must be called before using SDK )
            sdk.UF_InitSysParameter();
            ret = sdk.UF_InitCommPort("/dev/ttyACM0", 115200, false);
            Log.d("ret","detected usb" + ret);
            //ret = sdk.UF_InitCommPort("/dev/ttyACM0", 115200, false);
            if (ret == UF_RET_CODE.UF_ERR_CANNOT_OPEN_SERIAL){
                ret = sdk.UF_InitCommPort("/dev/ttyACM1", 115200, false);
                System.out.println("connect ttyACM1");
            }
            // Init Com Port ( This must be called before using SDK )
            //UF_RET_CODE result = sdk.UF_InitCommPort("/dev/ttyACM1", 115200, false);
            //result = sdk.UF_InitCommPort("/dev/ttyACM0", 115200, false);
            UF_RET_CODE result = sdk.UF_InitCommPort("/dev/bus/usb/005/002", 115200, false);
            result = sdk.UF_InitCommPort("/dev/ttyUSB0", 115200, false);
            result = sdk.UF_InitCommPort("/dev/bus/usb/001/004", 115200, false);
            display.append(result+"");
            // for UART connection

            //sdk.UF_InitCommPort("/dev/ttyS4", 115200, false);
            //sdk.UF_InitCommPort("/dev/ttyFIQ0", 115200, false);
        }
        sdk.UF_Reconnect();
...
//                            // UF_SendPacket
//                            /*UF_RET_CODE ret = sdk.UF_SendPacket((byte) 0x11, 0, 0, (byte) 0, 1000);
//                            Log.d("UF_SendPacket", ret.toString());
//
//
//                            byte[] receivedPacket = new byte[384];
//                            // UF_ReceivePacket
//                            ret = sdk.UF_ReceivePakcet(receivedPacket, 1000);
//                            Log.d("UF_ReceivePacket", ret.toString());
//                            Log.d("UF_ReceivePacket", Arrays.toString(receivedPacket));*/
...
//                    task.execute();
        ...
        Button verifyButton = findViewById(R.id.btnVerify);
...
//                    task.execute();
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            //Test_Identify();
                            //UF_SYS_PARAM[] parameters_set = new UF_SYS_PARAM[]{UF_SYS_PARAM.UF_SYS_FREE_SCAN, UF_SYS_PARAM.UF_SYS_TIMEOUT};
                            //int[] values_set = new int[]{0x30, 0x30};
                            //UF_RET_CODE ret = sdk.UF_SetMultiSysParameter(parameters_set.length, parameters_set, values_set);
                            //Log.d("UF_SetMultiSysParameter", ret.toString());
        ...
        cancelButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //UF_SYS_PARAM[] parameters_set = new UF_SYS_PARAM[]{ UF_SYS_PARAM.UF_SYS_TIMEOUT};
                //int[] values_set = new int[]{ 0x31};
                //UF_RET_CODE ret = sdk.UF_SetMultiSysParameter(parameters_set.length, parameters_set, values_set);
                // UF_SendPacket
                //UF_RET_CODE ret = sdk.UF_SendPacket((byte) 0xD0, 0, 0, (byte) 0, 10000);
                //ret = sdk.UF_SendPacket((byte) 0x1A, 0, 0, (byte) 0, 10000);
                //ret = sdk.UF_SendPacket((byte) 0x11, 0, 0, (byte) 0, 10000);
                ...
                sdk.UF_Cancel(false);
                //sdk.UF_InitSysParameter();
                //sdk.UF_SendPacket((byte) 0x62, 0, 0, (byte) 0x31,  1000);
                //sdk.UF_SetIdentifyCallback(identifyCallback);
                //sdk.UF_Reconnect();
            }
        });
